...
LetiFramework.Ui.decision.prototype = {
    ...
    preload: function() {
        ...
        for (var i = 0; i < this.decisions.length; i++) {
            ...
            if (decision.custom_font) {
                LetiFramework.Utils.loadAssetFont(decision.custom_font);
            }
        }
    ...
    create: function() {
        ...
        if (this.currentGamePageData.custom_font) {
            ...
            this.titleText = this.game.add.bitmapText(this.titleTextBg.x + this.titleTextBg.width / 2, this.titleTextBg.y + this.titleTextBg.height / 2, 
                cf.font, LetiFramework.Utils.wrapText(this.currentGamePageData.text, cf.wordWrapWidth, cf.font, cf.font_size), 
                cf.font_size);
        } else {
            this.titleText = this.game.add.text(this.titleTextBg.x + this.titleTextBg.width / 2,
                this.titleTextBg.y + this.titleTextBg.height / 2, this.currentGamePageData.text, this.currentGamePageData.text_style);
        }
        ...
        for (var i = 0; i < this.decisions.length; i++) {
            ...
            var scaleY = 1;
            if (decision.width) {
                scaleX = decision.width / decisionSprite.width;
            }
            if (decision.height) {
                scaleY = decision.height / decisionSprite.height;
            }
            decisionSprite.scale.setTo(scaleX, scaleY);
            ...
            if (decision.text.length > 0) {
                ...
                if (decision.custom_font) {
                    var cf = decision.custom_font;
                    this.decisionText = this.game.add.bitmapText(
                        decisionTextBg.x + decisionTextBg.width / 2, decisionTextBg.y + decisionTextBg.height / 2,
                        cf.font, LetiFramework.Utils.wrapText(decision.text, cf.wordWrapWidth, cf.font, cf.font_size), cf.font_size);
                } else {
                    this.decisionText = this.game.add.text(
                        decisionTextBg.x + decisionTextBg.width / 2, decisionTextBg.y + decisionTextBg.height / 2,
                        decision.text, decision.text_style);
                }
    ...
    transitionPage: function() {
        ...
        if (this.decisions[this.selectedChoice].next_page != null) {
            LetiFramework.GameController.nextGamePage = this.decisions[this.selectedChoice].next_page;
            LetiFramework.GameController.nextGamePageDataOverride = this.decisions[this.selectedChoice].next_page_override;
        } else if (this.currentGamePageData.next_pages && this.currentGamePageData.next_pages.length > this.selectedChoice) {
            LetiFramework.GameController.nextGamePage = this.currentGamePageData.next_pages[this.selectedChoice];
        }

        var points = this.decisions[this.selectedChoice].points;
        if (points) {
            var rs = LetiFramework.Db.readByKeysAndValues("scores", ["user_id", "game_id", "activity"], [this.currentUser.id, this.currentGame.id, this.currentGamePage + "_" + selection]);
        ...
        if (decisionBadge) {
            ...
            if (count == 1) {
                ...
                var rs = LetiFramework.Db.readByKeysAndValues(
                    "user_game_play", ["user_id", "game_id", "game_step"], [this.currentUser.id, this.currentGame.id, this.currentGamePage]);
                if (rs.length >= count) {
                    showBadge = true;
            ...
            if (showBadge) {
                ...
                var alreadyEarned = LetiFramework.Db.readByKeysAndValues("badges", ["user_id", "badge_id"], [this.currentUser.id, decisionBadge.id]).length > 0;
                if (alreadyEarned) {
                    LetiFramework.GameController.nextPage();
                } else {
                    var rs = jsonsql.query("select * from LetiFramework.GameController.currentStoryBadges where (id==" +
