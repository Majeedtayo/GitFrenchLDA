...
LetiFramework.Ui.register2 = function(game) {}

LetiFramework.Ui.register2.prototype = {
  init: function() {
  ...
  preload: function() {
    ...
    if (this.screenConfig.next_button.type == "image") {
      this.game.load.image(this.screenConfig.next_button.image, this.getScreenConfigImagePath(this.screenConfig.next_button.image));
    }
    ...
    if (this.nicknameField.field.background.type == 'image') {
      this.game.load.image(this.nicknameField.field.background.image, this.getScreenConfigImagePath(this.nicknameField.field.background.image));
    }
  ...
  preloadFields: function(field) {
    if (field.type == "choice") {
      this.game.load.image(field.selection_indicator.image, this.getScreenConfigImagePath(field.selection_indicator.image));

      if (field.choices.type == "image") {
        for (var j in field.choices.images) {
          var choice = field.choices.images[j];
          this.game.load.image(choice.image, this.getScreenConfigImagePath(choice.image));
        }
      } else {
        for (var j in field.choices.texts) {
          var choice = field.choices.texts[j];
          if (choice.background.type == "image") {
            this.game.load.image(choice.background.image, this.getScreenConfigImagePath(choice.background.image));
          }
        }
      }
    } else if (field.type == "text") {
      if (field.background.type == "image") {
        this.game.load.image(field.background.image, this.getScreenConfigImagePath(field.background.image));
      }
    }
  },
  create: function() {
    ...
    if (nextBtConfig.type == "image") {
      this.nextBt = this.game.add.sprite(nextBtConfig.position.x, nextBtConfig.position.y, nextBtConfig.image);
    ...
    if (regBtConfig.type == "image") {
      this.registerBt = this.game.add.sprite(regBtConfig.position.x, regBtConfig.position.y, regBtConfig.image);
    ...
    if (cancelBtConfig.type == "image") {
      this.cancelBt = this.game.add.sprite(cancelBtConfig.position.x, cancelBtConfig.position.y, cancelBtConfig.image);
    ...
    for (var i = 0; i < labelConfigs.length; i++) {
      ...
      if (this.currentField == 0 || fieldConfig.type == "text") {
        if (fieldConfig.background.type == "image") {
          var fieldBg = this.game.add.sprite(fieldConfig.position.x, fieldConfig.position.y, fieldConfig.background.image);
          ...
          var txt = this.currentField == 0 ? store.get("nickname") : store.get(currentFieldConfig.name || currentFieldConfig.fields[i].name);
          if (txt == null) {
            txt = "";
          }
          ...
          var self = this;
          (function() {
            var txt = self.currentField == 0 ? store.get("nickname") : store.get(currentFieldConfig.name || currentFieldConfig.fields[i].name);
            if (txt == null) {
              txt = "";
            }
        ...
        if (fieldConfig.selection_indicator.type == "image") {
          for (var i = 0; i < selCount; i++) {
            var selIndic = this.game.add.sprite(0, 0, fieldConfig.selection_indicator.image);
        ...
        if (choiceType == "text") {
          ...
          for (var i = 0; i < choices.images.length; i++) {
            ...
            var choiceImage = this.game.add.sprite(choice.position.x, choice.position.y, choice.image);
  ...
  nicknameHandler: function() {
    ...
    store.set("prevScreen", "Register2");
    store.set("maxLength", this.nicknameField.field.max_length);
  ...
  fieldLabelHandler: function(src) {
    ...
    store.set("inputType", fieldConfig.name);
    store.set("prevScreen", "Register2");
    store.set("maxLength", fieldConfig.field.max_length);
  ...
  validateField: function(fieldConfig) {
    ...
    if (fieldConfig.field.type == "choice") {
      ...
      if (this.selectedChoiceValues.length == 0 && validation && validation.required) {
        ...
        store.set(fieldConfig.name, this.selectedChoiceValues.length == 1 ?
      ...
      if ($.trim(fieldValue).length == 0 && validation && validation.required) {
        ...
        err = "Invalid " + fieldName;
      } else if (fieldConfig.confirm && store.get(fieldConfig.confirm) != fieldValue) {
  ...
  registerHandler: function() {
    ...
    LetiFramework.Analytics.trackEvent("Register", "Register", logObj, 0);
      ...
      LetiFramework.NetworkManager.postRequest("RegisterServlet",
        ...
        function(data) {
          ...
          if (data.recordId > 0) {
            ...
            LetiFramework.Analytics.trackEvent("Register", "Failed", message, 0);
            alert(message);
          }
        },
        function() {
          self.showError("Registration failed! Please check your internet connection.");
        });
    } else {
  ...
  clearFields: function() {
    ...
    for (var i = 0; i < this.otherRegisterFields.length; i++) {
      ...
      if (regField.fields) {
        for (var k in regField.fields) {
          store.remove(regField.fields[k].name);
        }
      } else {
