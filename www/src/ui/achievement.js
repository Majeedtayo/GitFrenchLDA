...
LetiFramework.Ui.achievement.prototype = {
	...
	preload: function() {
		...
		this.game.load.image(this.itemRow.facebook_icon.image, this.getScreenConfigImagePath(this.itemRow.facebook_icon.image));
		this.game.load.image(this.itemRow.twitter_icon.image, this.getScreenConfigImagePath(this.itemRow.twitter_icon.image));
	...
	create: function() {
		...
		if (titleArea.custom_font) {
			var title = this.game.add.bitmapText(this.game.world.centerX, panel2Height / 2 + panel2.y,
				titleArea.custom_font.font, LetiFramework.Utils.wrapText(titleArea.text, titleArea.custom_font.wordWrapWidth,
					titleArea.custom_font.font, titleArea.custom_font.font_size), titleArea.custom_font.font_size);
		...
		for (var i = 0; i < this.badges.length; i++) {
			(function(badge) {
				...
				if (self.itemRow.title_custom_font) {
					...
					badgeText = self.game.add.bitmapText(badgeSprite.x + badgeSprite.width + 5, badgeSprite.y, cf.font,
						LetiFramework.Utils.wrapText(badge.badge_text, cf.wordWrapWidth, cf.font, cf.font_size),
						cf.font_size);
				} else {
				...
				if (self.itemRow.message_custom_font) {
					...
					badgeMsg = self.game.add.bitmapText(badgeText.x, badgeText.y + badgeText.height, cf.font,
						LetiFramework.Utils.wrapText(badge.badge_action, cf.wordWrapWidth, cf.font, cf.font_size),
						cf.font_size);
				} else {
				...
				var twIcon = self.game.add.button(rowW - padding.right, 0.5 * rowH, self.itemRow.twitter_icon.image, function() {
				...
				var fbIcon = self.game.add.button(twIcon.x, twIcon.y, self.itemRow.facebook_icon.image, function() {
