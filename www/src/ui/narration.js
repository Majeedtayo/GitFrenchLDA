...
LetiFramework.Ui.narration.prototype = {
	...
	create: function() {
		...
		this.bg.events.onInputDown.add(function() {
			if (this.screenConfig.narration_side_panel && this.narrationTextShown) {
				...
				this.game.add.tween(this.scene).to({
					x: 0
		...
		if (this.narrations.length > 0) {
			...
			this.readBt = this.game.add.button(readBtPosition.x, readBtPosition.y, this.screenConfig.narration_side_panel.read_button.image, function() {
				...
				this.game.add.tween(this.scene).to({
					x: this.narrationTextShown ? narrW : 0
				}, 1000, Phaser.Easing.Exponential.Out, true);
			}, this, 2, 1, 0);
			...
			if (this.currentGamePageData.custom_font) {
				...
				this.narrationText = this.game.add.bitmapText(-halfNarrW, this.game.world.centerY, cf.font,
					LetiFramework.Utils.wrapText(this.narrations[this.narrationIdx], cf.wordWrapWidth, cf.font, cf.font_size),
					cf.font_size);
			} else {
				this.narrationText = this.game.add.text(-halfNarrW, this.game.world.centerY, this.narrations[this.narrationIdx],
					this.currentGamePageData.text_style);
			}
	...
	addComponents: function() {
		...
		for (var i = 0; i < components.length; i++) {
			...
			if (cmp && asset.link && !LetiFramework.GameController.debugging) {
				cmp.inputEnabled = true;
			...
			if (interactivity && cmp) {
				if (interactivity.action == "click" && !LetiFramework.GameController.debugging) {
					cmp.inputEnabled = true;
					cmp.input.useHandCursor = true;
					cmp.events.onInputDown.add(function(src) {
						var _this = LetiFramework.Renderer.currentState();
