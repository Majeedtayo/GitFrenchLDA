...
LetiFramework.Ui.register.prototype = {
    ...
    create: function() {
        ...
        for (var i = 0; i < this.otherRegisterFields.length; i++) {
            ...
            if (regField.drop_down == true) {
                ...
                fieldLbl.events.onInputDown.add(this.fieldLabelHandler, {
                    model: regField,
                    field: fieldLbl
                });
                dropDownBg.events.onInputDown.add(this.fieldLabelHandler, {
                    model: regField,
                    field: fieldLbl
                });
            } else {
    ...
    fieldLabelHandler: function() {
        ...
        if (this.model) {
            ...
            state.dd.height = model.drop_down_list_height + dividerHeight;
            ...
            mask.drawRect(model.position.x, model.position.y + model.height, model.width, model.drop_down_list_height + dividerHeight);
            state.dd.mask = mask;

            var ddBg = game.add.graphics(0, 0);
            ddBg.beginFill(model.drop_down_list_color, 1);
            ddBg.drawRect(0, 0, model.width, model.drop_down_list_height + dividerHeight);
            ddBg.endFill();
            ...
            state.dd.container.height = model.drop_down_list_height + dividerHeight;
            state.dd.add(state.dd.container);

            for (var i = 0; i < itemCount; i++) {
                (function(item) {
                    var optionBg = game.add.graphics(dividerHeight, i * (model.height + (i > 0 ? dividerHeight : 0)));
                    optionBg.beginFill(model.drop_down_list_item_color, 1);
                    optionBg.drawRect(0, 0, model.width - 2 * dividerHeight, model.height);
                    optionBg.endFill();
