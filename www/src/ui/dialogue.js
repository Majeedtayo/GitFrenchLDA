...
LetiFramework.Ui.dialogue.prototype = {
	...
	preload: function() {
		...
		for (var i = 0; i < this.dialogues.length; i++) {
			...
			if(dialogue.custom_font) {
				LetiFramework.Utils.loadAssetFont(dialogue.custom_font);
			}
		...
		if (!LetiFramework.App.isPhoneGap()) {
			...
			if (this.currentStoryData.audio && this.currentStorySound == null) {
				this.game.load.audio(this.currentStoryData.audio, this.getSoundPath(this.currentStoryData.audio));
			}
		}
	...
	create: function() {
		...
		this.homeBt = this.game.add.button(optionsBtPosition.x, this.currentGamePageData.show_menu_items ?
			this.screenConfig.options_button.children.home_button.position.y : optionsBtPosition.y, optionsDropDown.home_button.image,
			function() {
		...
		this.homeBt.alpha = this.currentGamePageData.show_menu_items ? 1 : 0;

		this.soundBt = this.game.add.button(optionsBtPosition.x, this.currentGamePageData.show_menu_items ?
			this.screenConfig.options_button.children.sound_button.position.y : optionsBtPosition.y,
		...
		this.soundBt.alpha = this.currentGamePageData.show_menu_items ? 1 : 0;

		this.musicBt = this.game.add.button(optionsBtPosition.x, this.currentGamePageData.show_menu_items ?
			this.screenConfig.options_button.children.music_button.position.y : optionsBtPosition.y,
		...
		this.musicBt.alpha = this.currentGamePageData.show_menu_items ? 1 : 0;
		...
		if (this.currentGamePageData.text.length > 0) {
			...
			if (this.currentGamePageData.custom_font) {
				...
				this.narrText = this.game.add.bitmapText(this.narrTextBg.x + this.narrTextBg.width / 2, this.narrTextBg.y + this.narrTextBg.height / 2,
					cf.font, LetiFramework.Utils.wrapText(this.currentGamePageData.text, cf.wordWrapWidth, cf.font, cf.font_size), cf.font_size);
			} else {
		...
		if (currentStoryData.audio && this.currentStorySound == null) {
			if (LetiFramework.App.isPhoneGap()) {
				this.currentStorySound = LetiFramework.SoundManager.getSound(
					this.getSoundPath(currentStoryData.audio), true);
			} else {
				this.currentStorySound = this.game.add.audio(
					currentStoryData.audio, 1, true);
			}
	...
	addHud: function() {
		if (this.hud.visible && !this.currentGamePageData.hide_hud) {
			...
			if (this.hud.label_custom_font) {
				...
				scoreLabel = this.game.add.bitmapText(0, 0, cf.font, "Score:", cf.font_size);
			} else {
			...
			if (this.hud.text_custom_font) {
				...
				scoreText = this.game.add.bitmapText(scoreLabel.width + 10, 0, cf.font, "" + this.currentUser.points, cf.font_size);
			} else {
			...
			if (this.currentBadge) {
				...
				if (this.hud.label_custom_font) {
					var cf = this.hud.label_custom_font;
					badgeLabel = this.game.add.bitmapText(scoreText.x + scoreText.width + 20, 0, cf.font, "Current Badge:", cf.font_size);
				} else {
	...
	nextDialogue: function() {
		...
		if (this.dialogueIdx < this.dialogues.length) {
			...
			var text_style = dialogue.text_style;
			if (text_style) {
				text_style.wordWrap = true;
				text_style.wordWrapWidth = text_style.width || (dialogue.bubble.width - 20);
			}

			var text_pos = dialogue.text_position;
			...
			if (text_pos) {
				var scaleX = 1;
				var scaleY = 1;
				if (dialogue.bubble.width) {
					scaleX = dialogue.bubble.width / this.speechBubble.width;
				}
				if (dialogue.bubble.height) {
					scaleY = dialogue.bubble.height / this.speechBubble.height;
				}
				this.speechBubble.scale.setTo(scaleX, scaleY);

				if (text_style && !text_style.width && !dialogue.bubble.width) {
					text_style.wordWrapWidth = this.speechBubble.width - 20;
				}

				if(dialogue.custom_font) {
					var cf = dialogue.custom_font;
					this.speechText = this.game.add.bitmapText(text_pos.x, text_pos.y, 
						cf.font, LetiFramework.Utils.wrapText(dialogue.text, cf.wordWrapWidth, cf.font, cf.font_size), cf.font_size);
				} else {
					this.speechText = this.game.add.text(text_pos.x, text_pos.y, dialogue.text, text_style);
				}				
			} else {
				if (text_style.width) {
					text_style.padding = text_style.padding || [0, 0, 0, 0];
					var pL = text_style.padding[0];
					var pT = text_style.padding[1];
					var pR = text_style.padding[2];
					var pB = text_style.padding[3];
					this.speechText = this.game.add.text(0, 0, dialogue.text, text_style);
					this.speechBubble.scale.setTo((text_style.width + pL + pR) / this.speechBubble.width,
					...
					if (dialogue.bubble.padding.constructor === Array) {
						var pLR = dialogue.bubble.padding[0];
						var pT = dialogue.bubble.padding[1];
						...
						text_style.wordWrapWidth = this.speechBubble.width - pLR * 2;
						this.speechText = this.game.add.text(this.speechBubble.x + this.speechBubble.width / 2,
							this.speechBubble.y + pT, dialogue.text, text_style);
						this.speechText.anchor.set(0.5, 0);
					} else {
						this.speechBubble.scale.setTo(
							dialogue.bubble.width / this.speechBubble.width, dialogue.bubble.height / this.speechBubble.height);
						text_style.wordWrapWidth = this.speechBubble.width - dialogue.bubble.padding * 2;
						this.speechText = this.game.add.text(this.speechBubble.x + this.speechBubble.width / 2,
							this.speechBubble.y + dialogue.bubble.padding, dialogue.text, text_style);
						this.speechText.anchor.set(0.5, 0);
					}
				}
			}
			...
			if (LetiFramework.GameController.debugging) {
				...
				this.dialogueTextGrp.setAll('inputEnabled', true);

				//  And allow them all to be dragged
				this.dialogueTextGrp.callAll('input.enableDrag', 'input');

				this.speechBubble.events.onDragStop.add(this.onDragStop, dialogue.bubble.position);
				this.speechText.events.onDragStop.add(this.onDragStop, this);
			}
			...
			if (dialogue.bubble.animation) {
				...
				if (anim.type == "fadeIn") {
					this.game.add.tween(this.dialogueTextGrp).from({
						alpha: 0
					}, anim.duration * this.animScale, Phaser.Easing.Linear.None, true);
			...
			if (this.dialogueIdx == this.dialogues.length - 1) {
				...
				if (this.currentGamePageData.next_page_button && this.currentGamePageData.next_page_button.enabled == false) return;
	...
	addComponents: function(componentsToAdd) {
		...
		for (var i = 0; i < components.length; i++) {
			...
			if (asset.type == "shape") {
				...
				if (asset.custom_font) {
					var txt = LetiFramework.Utils.wrapText(asset.text, asset.custom_font.wordWrapWidth,
						asset.custom_font.font, asset.custom_font.font_size);
					cmp = this.game.add.bitmapText(position.x, position.y, asset.custom_font.font, txt, asset.custom_font.font_size);
				} else {
			...
			if (LetiFramework.GameController.debugging && cmp) {
				cmp.events.onDragStop.add(this.onDragStop, this);
			}
		...
		if (LetiFramework.GameController.debugging) {
			//  Make them all input enabled
			this.componentsGroup.setAll('inputEnabled', true);

			//  And allow them all to be dragged
			this.componentsGroup.callAll('input.enableDrag', 'input');
		}
