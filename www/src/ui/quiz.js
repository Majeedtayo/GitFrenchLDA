			...
			if (!this.currentGamePageData.result) {
				...
				for (var i = 0; i < result.length; i++) {
					...
					if (this.currentGamePageData.result.custom_font) {
						...
						txt = this.game.add.bitmapText(this.game.world.centerX, startY + i * 50, cf.font,
							LetiFramework.Utils.wrapText(token, cf.wordWrapWidth, cf.font, cf.font_size),
							cf.font_size);
					} else {
	...
	showQuestion: function() {
		...
		if (question.type == "text") {
			...
			if (question.custom_font) {
				...
				questionText = this.game.add.bitmapText(
					question.position.x, question.position.y, cf.font,
					LetiFramework.Utils.wrapText(question.text, cf.wordWrapWidth, cf.font, cf.font_size), cf.font_size);
			} else {
				questionText = this.game.add.text(
		...
		if (choiceType == "text") {
			...
			for (var i = 0; i < choices.length; i++) {
				...
				var choiceText;
				if (choiceCustomFont) {
					choiceText = this.game.add.bitmapText(
						choice.position.x, choice.position.y, choiceCustomFont.font,
						LetiFramework.Utils.wrapText(choice.text, choiceCustomFont.wordWrapWidth, choiceCustomFont.font, choiceCustomFont.font_size),
						choiceCustomFont.font_size);
				} else {
		...
		if (result.type == "image") {
			...
			if (result.correct_text.custom_font) {
				...
				this.result = this.game.add.bitmapText(
					result.position.x, result.position.y, cf.font,
					LetiFramework.Utils.wrapText(result.correct_text.text, cf.wordWrapWidth, cf.font, cf.font_size), cf.font_size);
			} else {
			...
			if (correct) {
				...
				if (result.type == "image") {
					...
					if (result.wrong_text.custom_font) {
						...
						this.result = this.game.add.bitmapText(
							result.position.x, result.position.y, cf.font,
							LetiFramework.Utils.wrapText(result.wrong_text.text, cf.wordWrapWidth, cf.font, cf.font_size), cf.font_size);
					} else {
