...
LetiFramework.Ui.editpassword.prototype = {
	...
	preload: function() {
		...
		if (this.screenConfig.save_button.type == "image") {
			this.game.load.image(this.screenConfig.save_button.image, this.getScreenConfigImagePath(this.screenConfig.save_button.image));
		}

		if (this.currentPasswordConfig.background.type == "image") {
			this.game.load.image(this.currentPasswordConfig.background.image,
				this.getScreenConfigImagePath(this.currentPasswordConfig.background.image));
		}

		if (this.passwordConfig.background.type == "image") {
			this.game.load.image(this.passwordConfig.background.image,
				this.getScreenConfigImagePath(this.passwordConfig.background.image));
		}

		if (this.confirmPasswordConfig.background.type == "image") {
			this.game.load.image(this.confirmPasswordConfig.background.image,
				this.getScreenConfigImagePath(this.confirmPasswordConfig.background.image));
		}
	...
	create: function() {
		...
		if (saveBtConfig.type == "image") {
			this.saveBt = this.game.add.sprite(saveBtConfig.position.x, saveBtConfig.position.y, saveBtConfig.image);
	...
	fieldLabelHandler: function(src) {
		store.set("inputType", src.fieldName);
		store.set("prevScreen", "EditPassword");
		store.set("maxLength", src.maxLen);
		LetiFramework.Renderer.render("Keyboard");
	},
	saveHandler: function() {
		LetiFramework.Analytics.trackEvent("Edit Password", "Button Click", "Save", 0);
		var currentPassword = store.get("Current Password") || "";
		var newPassword = store.get("New Password") || "";
		var confirmPassword = store.get("Confirm Password") || "";
	...
	clearFields: function() {
		store.remove("Current Password");
		store.remove("New Password");
		store.remove("Confirm Password");
	},
